...
enum ParseType { Import, ImportBlockComment, Header, Body, Empty }
...
final FILE_GRAMMAR = (FILE_HEADER.optional() &
        IMPORT_BLOCK.castList<ParserOutput>().optional() &
        FILE_BODY.optional())
    .end();

// Parser for File Header
final FILE_HEADER = IMPORT_BLOCK
    .neg()
    .star()
    .flatten()
    .trimWithComments()
    .map((value) => ParserOutput(ParseType.Header, value));

// Parser for File Body
final FILE_BODY =
    any().star().flatten().map((value) => ParserOutput(ParseType.Body, value));

// Parser to Structured Import Mapping
final IMPORT_BLOCK = (IMPORT_STATEMENT & (IMPORT_STATEMENT | COMMENT).star())
    .map(_concat)
    .castList<ParserOutput>();

// Import Statement Grammar
final IMPORT_STATEMENT =
    (IMPORT & URI & ALIAS.optional() & COMBINATOR_LIST.optional() & SEMI_COLON)
        .flatten()
        .map(mapImportStatement);
...
// Comment Grammar
final COMMENT = (_LINE_COMMENT | _BLOCK_COMMENT).flatten().map((value) =>
    ParserOutput(
        ParseType.ImportBlockComment,
        value
            .trim())); // This is .trim() on String, so does not need .trimWithComments()
final _LINE_COMMENT =
    LINE_COMMENT_MARKER & NEWLINE.neg().star() & NEWLINE.optional();
final _BLOCK_COMMENT =
    BLOCK_COMMENT_START & BLOCK_COMMENT_END.neg().star() & BLOCK_COMMENT_END;
...
final SINGLE_LINE_STRING1 =
    (SINGLE_QUOTE & SINGLE_QUOTE.neg().star() & SINGLE_QUOTE)
        .flatten()
        .trimWithComments();
final SINGLE_LINE_STRING2 =
    (DOUBLE_QUOTE & DOUBLE_QUOTE.neg().star() & DOUBLE_QUOTE)
        .flatten()
        .trimWithComments();
...
final _IDENTIFIER_GRAMMAR =
    (_IDENTIFIER_START & _IDENTIFIER_PART.star()).flatten();
...
const rcrText =
    '($dartEmojis|$dartNoEmojis|$flutterEmojis|$flutterNoEmojis|$packageEmojis|$packageNoEmojis|$projectEmojis|$projectNoEmojis)';
final removeCommentRegex = RegExp(rcrText);
ParserOutput mapImportStatement(final String importPieces) {
  return ParserOutput(
      ParseType.Import, importPieces.replaceAll(removeCommentRegex, '').trim());
}
